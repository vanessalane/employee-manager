                    ...
                    connection.query(
                        ...
                        function(err, res) {
                            ...
                            inquirer.prompt({
                                ...
                                // limit this to the managers that are within the same department!
                                connection.query(
                                    `SELECT
                                    ...
                                    WHERE
                                        employee.manager_id IS NOT NULL AND ?`,
                                    { "role.department_id" : departmentId },
                                    function(err, res) {
                                        if (err) throw err;

                                        // surface the options to the user
                                        const managerChoices = res.map((res) => `${res.manager_name} - ${res.title}`)
                                        ...
                                        inquirer.prompt({
                                            ...
                                            if (answer.manager != 'None') {
                                                const managerChoiceIndex = managerChoices.indexOf(answer.manager);
                                                const manager_id = res[managerChoiceIndex].employee_id;
